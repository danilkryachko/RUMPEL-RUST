...
fn test_world_to_global_block_conversion() {
    let pos = WorldPos {
        position: Vec3::new(1.5, -0.5, 3.9),
    };
    assert_eq!(pos.to_global_block_pos(), IVec3::new(1, -1, 3));
}

#[test]
fn test_world_block_pos_round_trip() {
    let raw = IVec3::new(-12, 42, 7);
    let typed = WorldBlockPos::from(raw);

    assert_eq!(typed.position, raw);
    assert_eq!(IVec3::from(typed), raw);
}
