...
@group(1) @binding(0) var<storage, read> quads: array<vec4<u32>>;
@group(1) @binding(1) var<storage, read> draw_params: array<vec4<f32>>;
@group(1) @binding(2) var<storage, read> block_tiles: array<vec4<u32>>;
...
fn vertex(
    ...
    let chunk_offset = draw_params[instance_index];
    let base_quad_offset = u32(chunk_offset.w);
    ...
    // Build the 3D local position based on the face sweeping direction
    // Face mapping matches PackedVoxelFace enum values:
...
fn apply_distance_fog(color: vec4<f32>, world_position: vec3<f32>) -> vec4<f32> {
    let camera_position = view.camera_position_and_fog_start.xyz;
    let fog_start = view.camera_position_and_fog_start.w;
    let fog_end = max(view.fog_color_and_end.w, fog_start + 1.0);
    let camera_distance = distance(world_position, camera_position);
    let fog = smoothstep(fog_start, fog_end, camera_distance) * 0.50;
    let fogged = mix(color.rgb, view.fog_color_and_end.rgb, fog);
